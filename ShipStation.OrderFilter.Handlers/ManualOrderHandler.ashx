<%@ WebHandler Language="C#" Class="Ignia.ShipStation.ManualOrderHandler" %>
/*==============================================================================================================================
| Author        Ignia, LLC
| Client        Ignia, LLC
| Project       ShipStation Manual Order Filter
\=============================================================================================================================*/
using System;
...
namespace Ignia.ShipStation {
  ...
  \---------------------------------------------------------------------------------------------------------------------------*/
  public class ManualOrderHandler : HttpTaskAsyncHandler {
    ...
    public string LogLocation {
      get {
        ...
        _logLocation            = System.IO.Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"..\..\LogFiles\Application\ShipStation\ManualOrderHandlerLog_" + pacificNow.ToString("yyyyMMdd") + ".txt");
        // Ensure file exists
    ...
    public override async Task ProcessRequestAsync(HttpContext context) {
      ...
      /*------------------------------------------------------------------------------------------------------------------------
      | Establish variables for ShipStation API call(s)
      \-----------------------------------------------------------------------------------------------------------------------*/
      string resourceUrl        = "";
      ...
      if (capturedException != null) {
        await WriteLog("ShipStation orders metadata parse error", capturedException.SourceException.Message);
      ...
      try {
        ...
        for (int i = pagesToProcess; i > 0; i--) {
          ...
          await WriteLog("Retrieving orders", ordersEndpoint);

          // Get orders payload for specific page
          using (HttpClient) {
            using (var response = await HttpClient.GetAsync(ordersEndpoint)) {
              ordersResult              = await response.Content.ReadAsStringAsync();
            }
          }

          // If response is unavailable, exit
          if (String.IsNullOrEmpty(ordersResult)) {
            await WriteLog("ShipStation Orders response error", "Response body not available.");
            return;
          }

          // Log the response
          await WriteLog("GET orders endpoint response", ordersResult);

          // Deserialize the response
          dynamic openOrders            = JObject.Parse(ordersResult);

          // Exit if the response does not contain order(s) information
    ...
    private async Task<string> ModifyOrdersAsync(dynamic orders) {
      ...
      foreach (dynamic order in orders) {
        ...
        for (int i=0; i<order.items.Count; i++) {
          ...
          /*--------------------------------------------------------------------------------------------------------------------
          | Remove individual item weight property, per ShipStation Support suggestion, so that total order weight modification
          | takes precedence
          \-------------------------------------------------------------------------------------------------------------------*/
          //item.Property("weight").Remove();
